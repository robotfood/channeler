...
function selectedProfiles() {
  ...
  return hasArg('--all') ? [...base, 'qsv_4k', 'smooth_1080p60'] : base
}
...
function profileBackends(profile: string, backends: Backend[]) {
  if (HARDWARE_PROFILES.includes(profile as typeof HARDWARE_PROFILES[number])) return backends
  return ['cpu'] as Backend[]
}
